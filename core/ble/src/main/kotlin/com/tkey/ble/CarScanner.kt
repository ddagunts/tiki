...
class CarScanner(private val ctx: Context) {
    ...
    fun discover(vin: String): Flow<Event> = callbackFlow {
            ...
            override fun onReceive(c: Context, intent: Intent) {
                when (intent.action) {
                    BluetoothDevice.ACTION_FOUND -> {
                        ...
                        if (device != null) {
                            val beacon = Beacon(device.address, name ?: "(no name)", rssi.toInt())
                            Log.i(TAG, "FOUND ${beacon.address} name='$name' rssi=$rssi")
                            trySend(if (name == target) Event.Match(beacon) else Event.Other(beacon))
                        }
                    }
